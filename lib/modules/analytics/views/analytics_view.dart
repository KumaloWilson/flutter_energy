...
class AnalyticsView extends StatelessWidget {
        ...
        child: Obx(() {
          ...
          return ListView(
            ...
            children: [
              ...
              _buildEnergyOverviewCard(controller, context),

              const SizedBox(height: 24),

              // Device Selection
              _buildDeviceSelector(controller, context),

              const SizedBox(height: 16),

              // Daily Predictions Card
              _buildPredictionsCard(controller, context),

              const SizedBox(height: 24),

              // Peak Demand Card
              _buildPeakDemandCard(controller, context),

              const SizedBox(height: 24),

              // Weekly Usage Chart
              _buildWeeklyUsageCard(controller, context),

              const SizedBox(height: 24),

              // Device Breakdown
              _buildDeviceBreakdownCard(controller, context),
            ],
          );
        }),
      ),
  ...
  Widget _buildEnergyOverviewCard(AnalyticsController controller, BuildContext context) {
    final theme = Theme.of(context);
  ...
  Widget _buildDeviceSelector(AnalyticsController controller, BuildContext context) {
    final theme = Theme.of(context);
  ...
  Widget _buildPredictionsCard(AnalyticsController controller, BuildContext context) {
    final theme = Theme.of(context);
          ...
          children: [
            ...
            Obx(() {
              final selectedDevice = controller.devices.firstWhere(
                ...
                orElse: () => controller.devices.isNotEmpty ? controller.devices.first : ApplianceInfo(
                  id: 0,
                  appliance: 'Unknown',
                  ratedPower: '0 W',
  ...
  Widget _buildPeakDemandCard(AnalyticsController controller, BuildContext context) {
    final theme = Theme.of(context);
  ...
  Widget _buildWeeklyUsageCard(AnalyticsController controller, BuildContext context) {
    final theme = Theme.of(context);
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(
              height: 200,
              child: Obx(() {
                final dailyData = controller.getDailyData();
                        ...
                        sideTitles: SideTitles(
                          ...
                          getTitlesWidget: (value, meta) {
                            ...
                            if (index >= 0 && index < dailyData.length) {
                              final date = dailyData[index].day;
                              return SideTitleWidget(
                                meta: meta,
                                child: Text(
            ...
            Obx(() {
              ...
              final peakDay = maxIndex >= 0 && dailyData[maxIndex].day != null
                  ? DateFormat('EEEE').format(dailyData[maxIndex].day!)
                  : 'Unknown';
  ...
  Widget _buildDeviceBreakdownCard(AnalyticsController controller, BuildContext context) {
    final theme = Theme.of(context);
          ...
          children: [
            ...
            SizedBox(
              ...
              child: Obx(() {
                ...
                // In a real app, this would use actual consumption data
                // Here we're simulating based on rated power
                double totalPower = 0;
                final pieData = <PieData>[];

                for (final device in controller.devices) {
                  final powerValue = controller.parseWattage(device.ratedPower).toDouble();
